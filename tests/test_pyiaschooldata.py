...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = ia.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pyiaschooldata as ia
        assert hasattr(ia, 'fetch_enr')
        assert hasattr(ia, 'fetch_enr_multi')
        assert hasattr(ia, 'tidy_enr')
        assert hasattr(ia, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pyiaschooldata as ia
        assert hasattr(ia, '__version__')
        assert isinstance(ia.__version__, str)
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = ia.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        if 'is_district' in df.columns and 'grade_level' in df.columns:
            ...
            if len(total_df) > 0:
                ...
                # Iowa should have ~510k students total across districts
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = ia.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = ia.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pyiaschooldata as ia
        years = get_test_years()
        df_single = ia.fetch_enr(years['max_year'])
        df_multi = ia.fetch_enr_multi([years['max_year']])
