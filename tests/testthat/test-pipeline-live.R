...
test_that("Iowa DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Iowa DOE URL and verify HTTP 200")
})
...
test_that("Can download Iowa enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Iowa enrollment file", {
  skip_if_offline()
...
test_that("iaschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- iaschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- iaschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- iaschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- iaschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- iaschooldata::fetch_enr(test_year, tidy = TRUE)
